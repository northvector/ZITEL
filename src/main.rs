...
async fn get_index_data_loop(token: &str, auth_header: &str) -> Result<(), Box<dyn Error>> {
    ...
    const TOTAL_PAGES: u8 = 4; // Network Info, Connection, Cell Info, IP Config, Data Usage, System Info
    ...
    loop {
        ...
        match current_page {
            1 => display_network_info(&response),
            2 => display_connection_info(&response),
            3 => display_cell_info(&response),
            4 => display_ip_config(&response),
